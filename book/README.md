...
- `core`：核心子域。
- `core.presentation` 核心子域展现层，负责对外暴露 REST 接口、接收请求参数并返回响应对象，不承载业务规则。
- `core.application` 核心子域应用层，负责编排用例流程，协调领域对象、领域服务和仓储接口完成业务动作。
- `core.domain` 核心子域领域层，承载领域模型，应避免依赖技术实现。
